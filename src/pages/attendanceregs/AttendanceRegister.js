...
import { ToastAlert,IsLoggedIn, MessageComponent } from '../Functions';
...
export function AttendanceRegister(){
 ...
 return numbOfSms
      ...
      window.location.href="#"
         ...
         fetch(`/attendanceregs/${registerParams.registrar}/${registerParams.id}`).then(res=>res.json()).then(resp=>{
          setAttendanceRegister(resp)
          ...
          ;
            ...
            setRegistrarName(resp[0].name)
             setInstitution(resp[0].institution)
             setRegisterTitle(resp[0].registerTitle)
             setRegistrarContact(resp[0].contact)
       
            setMessageesNumb(resp[0].attendees.length)
         setArrayOfAttendees(resp[0].attendees)
     ...
     function DisplayContacts(registerParams){
      ...
      if(parseInt(cookies.user.contact)==parseInt(registerParams.registrar)){
        ...
        fetch(`/attendees/${registerParams.registrar}/${registerParams.id}`).then(res=>res.json()).then(res=>{
          ...
          let responseDoc=res[0],position=1,registerDoc=responseDoc
...
<textarea row="1" type="text" class="form-control" autoComplete="off" name="name" ></textarea><br></br>
 ...
 */}
 ...
 {(()=>{
  if(attendanceRegister){
    if(attendanceRegister.length==0){
      return(<MessageComponent message="This register does not exist"/>)
    }else{
      return(
        <div>
...
<div>{messagees}</div>
    ...
    return(<MessageComponent message="Please wait....."/>)
  }
 })()}
